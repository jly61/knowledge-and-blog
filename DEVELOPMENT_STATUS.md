...
- ✅ v2.0 知识库问答（RAG）
  ...
  - ✅ 错误处理优化（不影响笔记创建/更新）
- ✅ AI 自动生成摘要和 SEO
  - ✅ SEO 生成 Server Action（app/actions/ai/seo.ts）
  - ✅ 发布表单 AI 生成按钮（components/posts/publish-note-client.tsx）
  - ✅ 文章编辑器 AI 生成按钮（components/posts/post-editor.tsx）
  - ✅ 支持 OpenAI 和 Ollama 本地模型
...
- ✅ AI 双向链接建议
  - ✅ 链接建议 Server Action（app/actions/ai/links.ts）
  - ✅ 链接建议组件（components/editor/ai-link-suggestions.tsx）
  - ✅ 编辑器集成（components/editor/markdown-split-editor.tsx）
  - ✅ 基于向量相似度推荐相关笔记
  - ✅ 输入 `[[` 时自动触发建议
...
### 优先级 1：AI 功能增强
1. ✅ v1.0 对话侧边栏（已完成）
2. ✅ v2.0 知识库问答（RAG）（已完成）
3. **AI 自动生成摘要和 SEO**（推荐优先开发）
   - 发布文章时自动生成摘要
   - 自动生成 SEO 关键词和描述
   - 自动优化标题和元数据
4. **AI 自动标签推荐**（推荐优先开发）
